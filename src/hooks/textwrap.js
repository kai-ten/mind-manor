    ...
    wrap.tspans = function(text, dimensions, padding) {
        ...
        while (pieces.length > 0) {
            ...
            if (line_width > dimensions.width) {
                ...
                // FIGURE OUT AN APPROXIMATION METHOD? (Seems like a bell curve, get the computed text length for each size and graph points)
                if (tspan.node().getComputedTextLength() < 200) {
                    x_offset = (tspan.node().getComputedTextLength() * -.85);
                } else if (tspan.node().getComputedTextLength() < 175) {
                    x_offset = (tspan.node().getComputedTextLength() * -.78);
                } else if (tspan.node().getComputedTextLength() < 150) {
                    x_offset = (tspan.node().getComputedTextLength() * -.70);
                } else {
