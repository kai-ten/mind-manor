var GraphCreator = function(svg, nodes, links){
    ...
    var dragSvg = d3.zoom()
          ...
          .on("start", function(){
            ...
            if (!this) d3.select('body').style("cursor", "move");
          })
          .on("end", function(){
  ...
  GraphCreator.prototype.dragmove = function(d, event) {
    var thisGraph = this;
    if (thisGraph.state.shiftNodeDrag){
      thisGraph.dragLine.attr('d', 'M' + d.x + ',' + d.y + 'L' + d3.pointer(thisGraph.svgG.node())[0] + ',' + d3.pointer(this.svgG.node())[1]);
    } else{
      d.x += 1000;
      d.y += 1000;
      thisGraph.updateGraph();
  ...
  GraphCreator.prototype.pathMouseDown = function(d3path, d, event){
    var thisGraph = this,
  ...
  // mousedown on node
  GraphCreator.prototype.circleMouseDown = function(d3node, d, event){
    var thisGraph = this,
        state = thisGraph.state;
    event.stopPropagation();
  ...
  GraphCreator.prototype.changeTextOfNode = function(d3node, d, event){
    var thisGraph= this,
  ...
  // mouseup on nodes
  GraphCreator.prototype.circleMouseUp = function(d3node, d, event){
    var thisGraph = this,
        state = thisGraph.state,
        consts = thisGraph.consts;
    // reset the states
  ...
  // mouseup on main svg
  GraphCreator.prototype.svgMouseUp = function(event){
    var thisGraph = this,
        state = thisGraph.state;
    if (state.justScaleTransGraph) {
      ...
      var xycoords = d3.pointer(thisGraph.svgG.node()),
  ...
  GraphCreator.prototype.updateGraph = function(){
    ...
    thisGraph.circles = thisGraph.circles.data(thisGraph.nodes, function(d){ console.log(d); return d;});
    thisGraph.circles.attr("transform", function(d, event){return "translate(" + event.x + "," + event.y + ")";});
    ...
    newGs.classed(consts.circleGClass, true)
      .attr("transform", function(d, event){console.log(d); return "translate(" + event.x + "," + event.y + ")";})
  ...
  /** MAIN SVG **/
  const links = graphInfo.links.map(d => {
    return Object.assign({}, d);
  });
  const nodes = graphInfo.nodes.map(d => {
    return Object.assign({}, d);
  });
